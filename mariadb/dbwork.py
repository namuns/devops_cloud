...
def selectData():
    ...
    # 1. 리스트박스 초기화를 시킨다. 화면을 안지워주면 밑에 붙게 되어있다.
    # gui에서 선언한 list입니다. 기존값을 지워줘서 검색한 값만 나오도록 해주었습니다.
    listcode.delete(0, listcode.size() -1)
    listName.delete(0, listName.size() -1)
    listprice.delete(0, listprice.size() -1)
    listsales.delete(0, listsales.size() -1)
...
def calculate1():
    ...
    # 회원 정보 select 기능 구현
    sql = "SELECT (pbr/price*100) AS 'BPS 구하기' from stocktbl ORDER BY code ASC"
...
def calculate2():
    ...
    # 회원 정보 select 기능 구현
    sql = "SELECT (dps/price*100) AS 'BPS 구하기' from stocktbl ORDER BY code ASC"
...
# 텍스트박스와 라벨 "코드"
label1 = Label(editFrame, text="코드")
...
btnBack.pack(side=LEFT, padx=10, pady=10)
btnBack = Button(listFrame, text="배당률 계산", command=calculate2)
btnBack.pack(side=LEFT, padx=10, pady=10)
